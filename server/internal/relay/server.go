...
type Server struct {
	...
	Probe   *probe.Engine // 按需线路预热（line_stats 缺失/过期时异步补测）
...
func NewServer(st *store.Store, secret string, a *auth.Service, r *routing.Service, w *usage.Writer, pm *proxyman.Manager, cfg config.Config, bk *breaker.Engine, pb *probe.Engine) *Server {
	return &Server{
		Store: st, Secret: secret, Auth: a, Routing: r, Logs: w, PM: pm, Cfg: cfg, Breaker: bk, Probe: pb,
...
// 熔断过滤（FR-B1/B2，v1.5.45）：view 中熔断且冷却未到期的渠道整渠道跳过；
// 冷却已到期（半开）的渠道只放行首个组合并标记 trial（执行前需认领）；
// 按需线路预热（FR-S1）：参与尝试的渠道若 line_stats 缺失/过期，异步补一次
// 线路质量探测（流量驱动，无流量零成本）
func (s *Server) plan(matched, defaults []*routing.ResolvedChannel, model string, view breaker.View) []attempt {
	...
	for _, rc := range append(matched, defaults...) {
		...
		if s.Probe != nil {
			s.Probe.MaybeWarmup(rc.Channel) // 线路数据缺失/过期时异步预热（节流）
		}
