...
// 内置价目：公开定价的近似值，**统一折算为 USD**（每百万 token），管理员可在价目管理页修改；
// 种子按 model 主键冲突即跳过，不会覆盖已有改动。
// 注意：按实现期知识写入的近似值，非实时搜索，请按官方页核准。
var seedPricing = []ModelPricing{
	...
	// GLM / Kimi（官方 CNY 定价按 1 USD ≈ 7.2 CNY 折算为 USD，保证统计币种一致）
	...
	{Model: "moonshot-v1-128k", InputPerM: 8.33, OutputPerM: 8.33, Currency: "USD"},
}
