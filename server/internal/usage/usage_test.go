...
// 目录「关联价目」参与计价（v1.5.63，v1.5.64 调整优先级）：**关联价优先于一切名称
// 直查**（上游名/裸名条目仅作未关联时的兜底）；关联名未定价时回退名称直查
func TestCatalogAliasPricing(t *testing.T) {
	...
	catalogs := []store.CatalogModel{
		...
		{Name: "alias-x", PricingModel: "provider/flashx", Enabled: 1, CreatedAt: now, UpdatedAt: now},
	}
	...
	// 关联优先于裸名人工条目：按 provider/bare = 10（而非裸名 9+9=18）
	ic, oc = ComputeCost(db, "bare", "", "usd", 1, 0, u)
	if ic == nil || *ic+*oc != 10 {
		t.Fatalf("关联价应优先 $10，实际 %v / %v", ic, oc)
	}
	// 关联优先于上游名直接命中：alias-x → provider/flashx = 1.62（而非 upstream-x 2）
	ic, oc = ComputeCost(db, "alias-x", "upstream-x", "usd", 1, 0, u)
	if ic == nil || *ic+*oc != 1.62 {
		t.Fatalf("关联价应优先于上游名 $1.62，实际 %v / %v", ic, oc)
	}
	// 无关联的上游名直查不受影响：1 + 1 = 2
	ic, oc = ComputeCost(db, "plain", "upstream-x", "usd", 1, 0, u)
	if ic == nil || *ic+*oc != 2 {
		t.Fatalf("未关联模型应按上游名 $2，实际 %v / %v", ic, oc)
	}
	// 关联名未定价 → 回退裸名直查（无裸名 → 未定价）
	if ic, oc = ComputeCost(db, "orphan", "", "usd", 1, 0, u); ic != nil {
		t.Fatalf("关联名未定价且无裸名应返回 nil，实际 %v", *ic)
	}
