...
>   实例）；打开弹窗清空搜索词
> - v1.5.73：**长模型名显示不全修复**——①目录表「价格模型」行允许换行（去掉
>   nowrap 截断），单价列不再限宽 230（表格横向滚动承接）；②「关联价目」点选候选
>   下拉不再跟随输入框宽度（popupMatchSelectWidth=false）并去掉省略号截断，
>   长名（如 `openrouter/openai/gpt-5.6-luna`）完整展示
> - v1.5.72：**价目表被目录关联的条目置前显示**——恢复排序（按
>   `pricing_model`（空回退目录名）命中价目条目判定，v1.5.71 前的旧实现误按目录名
>   匹配），仍不打「目录」标签（v1.5.71 决定不变：映射在目录侧表达）；
>   提示文案改为「被模型目录关联的条目置前显示」
> - v1.5.71：**目录表单「模型名」改纯输入，价目表移除目录标注**——①目录与价目是
>   单一映射关系（目录名 → `pricing_model` → 价目条目），映射在模型目录侧表达即可：
>   「模型名」从价目表点选下拉改回**纯输入框**（与渠道请求的模型名一致，提示语同步
>   修正），「关联价目」点选下拉保留（候选带来源标签与价格摘要）；②价目表移除
>   「目录」标签、目录置前排序与对应提示文案，恢复纯按模型名排序，搜索 + 来源筛选不变
> - v1.5.70：**同步恢复自动新增，全量 upsert + 存量清空重灌**——v1.5.65 的
>   「只刷新不新增」使价目表只剩目录关联条目（11 条），新增模型点选/关联候选基本
>   无从选择，且存量 3298 条老同步/手工/种子混杂条目已与远程脱节。现按管理员决策
>   修订：**同步改全量 upsert**——远程源收录的条目缺失即新增（source 记命中源）、
>   已存在的远程来源条目与目录关联条目刷新四档价；**manual/import 且未被目录关联的
>   条目不覆盖、不删除**（后续人工新增的条目永不被同步动）；删除仍永久生效（同步
>   永不删除）。存量处理：删除全部 3298 条 manual 条目（含 z-ai/glm-5.2、
>   z-ai/glm-5.3 两条目录在用价目，删除前已导出备份），重启后同步自动灌入双源
>   全量条目（~7500 条）；目录中 `z-ai/*` 关联名远程源无此命名，需改关联
>   models.dev/LiteLLM 命名（如 `zhipuai/glm-5.2`）或重新手工补价。
>   同步结果改 {added, refreshed}，管理页提示「价目新增 N 条；刷新 M 条」
...
>   **存量来源回填**：`source` 列引入前的存量条目（老同步/手工/种子混杂，无法区分）
>   启动时统一回填 `manual`——按现行语义「价目表增长完全由管理员控制」，未挂远程
>   同步的条目即管理员维护；幂等（首启跑完即无空来源），后续同步/编辑按实际来源覆盖。
>   目录页 `glm-5.2/glm-5.3`（关联名 `z-ai/glm-5.2`、`z-ai/glm-5.3`）远程源均无此
>   命名、无法自动刷新，回填后显示「手工」；如需自动同步可改关联 models.dev 命名
>   （`zai/glm-5.2`、`zai/glm-5.3`），但远端美元价可能与现有手工维护价不同
...
>   来源由服务端裁定（仅接受 `import`，其余——含伪造远程源名——一律记 `manual`）；
>   价目表条目显示来源标签（手工/导入灰标、远程源紫标），筛选栏新增**来源过滤**
>   （全部 / LiteLLM / models.dev / 手工 / 导入），JSON 导入的条目记 `import`。
>   模型目录同步展示来源：目录表（用户页与管理员页共用 CatalogPrice）的
>   「价格模型」行附来源标签（`GET /api/admin/models` / 用户目录接口 DTO 增
>   `source`，取关联价目条目的来源）；目录表单「关联价目」两个点选候选与选中
>   预览同样附来源标签。
>   AutoMigrate 自动补列，存量条目来源为空、下次同步关联条目时自动补齐
...
>   **模型目录关联的价目条目**（`catalog_models.pricing_model` 引用）刷新为
>   LiteLLM/OpenRouter 最新价（双源合并，LiteLLM 先到先得），不新增任何模型；
>   未关联条目与远端源没有的关联名保持现状；关联名不在价目表仅计数提示（`missing`）。
>   价目表增长完全由管理员控制（手工录入 / JSON 导入 + 关联条目刷新）；管理页同步
>   提示改为「关联价目刷新 N 条 · M 个关联价目不在价目表」
> - v1.5.64：**计价只按目录关联 + 关联价目定时网络刷新**（价格链路 review 修订）——
>   ① 查价优先级改为**目录关联价目 → 上游模型名 → 入站模型名**：v1.5.63 的别名是
>   "裸名人工条目优先、只补缺"，与"关联了就用关联价"的预期相反，现快照持有
>   目录名→关联名映射，命中关联即按关联价计费（关联名未定价才回退名称直查），
>   上游名/裸名条目不再抢占；② 官方价目同步从「只补缺不覆盖」改为**「补缺 + 目录
>   关联条目刷新」**：被 `catalog_models.pricing_model` 引用的价目条目每次同步
>   （启动 + 周期 + 手动）用 LiteLLM/OpenRouter 最新价覆盖四档单价，其余已存在
>   条目保持不动，关联价保持新鲜且不被人工改动后失真；同步结果补 `refreshed`
>   计数并在管理页提示，顺带修复 `skippedExisting` 恒为 0 的统计缺失
> - v1.5.63：模型目录**「关联价目」参与费用计算**（修复：目录已关联价目但费用仍为 0）
>   ——`pricing_model` 此前仅用于目录页展示单价，费用计算只按日志 upstream_model/model
>   精确查价目表；同步源收录的是带前缀名（如 `openrouter/z-ai/glm-5.3-flashx`）时，
>   裸名请求永远未定价、费用估算为 0。现价目快照重建时把目录关联解析为**计价别名**：
>   目录名未直接命中价目表而关联名命中 → 按关联名价计费（只补缺不覆盖，裸名人工条目
>   优先）；写入时快照与统计补算同口径，历史未定价行自动按关联价补齐；目录新增/编辑/
>   删除即时失效价目缓存。查价优先级：上游模型名 → 入站模型名（含目录别名）
> - v1.5.62：**移除内置价目种子，修复删除价目后"复活"**——价目表删除的条目（如
>   模型目录改名后的老模型价）在重启后自动重现：`store.Open` 每次启动把硬编码的
>   内置价目种子（30+ 常见模型官方价）按"冲突跳过"重插，删掉的种子模型重启即回来。
>   现整段删除种子（`store/pricing.go`）：价目来源只剩管理员手工录入/JSON 导入/
>   官方价目远程同步；**新部署实例价目表为空**（费用显示未定价），需点"同步官方价目"
>   或导入/手工录入，已部署实例不受影响；启动路径不再写任何价目（测试固化：新库为空、
>   手工条目重开原样保留、删除后重开不补回）。官方价目同步保持「只补缺不覆盖」现状
>   不变——同步源里存在的模型删除后仍可能被补回（v1.5.61 决策：仅移除种子路径）
...
手动收录常用模型；价格模型可关联价目表中任意名称，留空时按目录名称匹配；**计价只按
关联价目**（v1.5.64：关联价优先于上游/裸名条目），**关联条目由官方价目同步定时从网络
刷新**（见 FR-M4.2）。目录是点选数据源，
不是路由依据——仅出现在渠道/模板表单的候选列表与用户模型页的目录视图中；删除目录项
不影响已引用它的渠道配置。

- FR-MC1 目录管理（管理员）：模型目录 CRUD（名称、备注、启用）；「模型名」为纯输入框
  （v1.5.71：与渠道请求的模型名一致，不再从价目表点选）、「关联价目」为单一映射的
  点选下拉（候选带来源标签与价格摘要，选中显示四档价预览）；停用的目录项不再出现
...
- FR-L5 费用口径：查价优先级 = **目录关联价目 → 上游实际模型名（经 model_mapping 映射后）
  → 入站模型名**（v1.5.64：模型目录中该模型关联的价目条目优先——只按关联计费；关联名
  未定价时才回退名称直查），写入日志时固化快照，避免价目调整后历史统计漂移。**缓存计价**（v0.7）：
  ```
...
- FR-M4 模型价目表：每模型设定**四档单价**（每百万 token，USD）：输入 / **缓存读输入** /
  **缓存写输入** / 输出；缓存档可留空（回退规则见 FR-L5）；支持手工编辑与
  JSON/CSV 批量导入；价目仅用于统计估算，**无内置种子价目**（v1.5.62 移除——
  此前启动种子会把删除的条目在重启后补回）：新部署实例价目表为空，经批量导入/
  手工录入/远程同步填充（见 FR-M4.2）。
  管理页表格支持**按模型名搜索筛选与按来源筛选**（v1.5.71：不再标注/置前"目录"
  ——目录与价目的单一映射关系在模型目录侧表达）
...
- FR-M4.2 官方价目远程同步：从 models.dev（api.json）与 LiteLLM
  ...
  同步语义（v1.5.70 修订）：
  **全量 upsert**——远程源收录的条目缺失即新增（source 记命中源）、已存在的远程来源
  条目与目录关联条目按网络最新价覆盖四档单价；manual/import 且未被目录关联的条目
  不覆盖、不删除（人工维护优先，后续手工新增的条目永不被同步动）；同步永不删除
  （删除永久生效）。同步结果 {added, refreshed}，管理页提示「价目新增 N 条；刷新 M 条」。
  ...
  价目条目标识**来源**（v1.5.68）：`source` 列（LiteLLM / models.dev / manual / import，
  历史数据为空），同步刷新写入命中源、管理接口录入记手工（服务端裁定）、JSON 导入记
  import；价目表条目显示来源标签，筛选栏支持按来源过滤
