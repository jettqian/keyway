...
- FR-T2.1 令牌列表的"限定渠道"列以**胶囊按钮**呈现摘要（边框 + 展开箭头 + hover 反馈、
  ...
  优先级规则，不再有"不限时按渠道优先级"的歧义。数据模型：`channel_order_json` 存
  全量顺序，`channel_ids_json` 存启用集合，二者分离；**关闭的渠道保持原位**（置灰
  显示，仅移出路由范围，顺序不变、跨会话持久），重新打开即恢复路由且优先级不变；
  **至少保留一个启用的渠道**——关闭最后一个启用渠道会被阻止并提示改用「吊销」
  （空集合会落回"不限"语义，停用令牌请吊销）；未加入的渠道（虚线行）打开开关后
  追加到列表末尾。存量"不限"令牌（启用集合为空，含创建表单留空）打开面板时以
  **全部渠道按优先级预览**（黄色提示条说明现状，不落库），首次任何调整即把令牌
  **固化**为所选渠道；后端"空=不限"数据语义保留仅作存量兼容。排序：拖拽**行首
  手柄**（@dnd-kit/sortable：按压移动 4px 才进入拖拽、名称/开关区域不可拖、其余行
  自动让位动画、触屏可用）或用 **↑↓ 按钮**逐位调整，与全局渠道优先级独立、互不
  影响其他令牌。面板内操作**乐观更新**并以接口返回结果**静默更新对应行**（不触发
  列表 loading 整表刷新）；保存失败回滚到操作前状态。列摘要显示前两个渠道名 + 总数；
  已吊销令牌仅显示摘要不可配置
...
| A24 | 令牌渠道控制 | 面板无"限定"主开关，令牌始终按列表顺序路由（一套优先级规则）；关闭某渠道开关后该令牌不再路由到它，且该渠道**保持原位**（置灰显示，顺序不变，刷新页面后仍在原位），重新打开后立即恢复路由且优先级不变；关闭最后一个启用渠道被阻止并提示改用「吊销」；拖拽行首手柄或 ↑↓ 改变顺序后，下一个请求按新顺序选择候选渠道，其他令牌不受影响；存量"不限"令牌打开面板显示按渠道优先级的预览（黄色提示），首次调整即固化为所选渠道；面板操作过程中弹层位置稳定不动 |
