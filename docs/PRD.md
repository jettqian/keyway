...
- FR-M4 模型价目表：每模型设定**四档单价**（每百万 token，USD）：输入 / **缓存读输入** /
  **缓存写输入** / 输出；缓存档可留空（回退规则见 FR-L5）；支持手工编辑与
  ...
  此前启动种子会把删除的条目在重启后补回）：新部署实例价目表为空，经远程同步
  （FR-M4.2）/批量导入/手工录入填充。
...
- FR-M4.2 官方价目远程同步：从 LiteLLM（model_prices_and_context_window.json）与
  ...
  仅 mode=chat / text→text 且输入输出价均大于 0 的条目）。同步语义（v1.5.64 修订）：
  **目录关联条目刷新 + 其余只补缺**——被 `catalog_models.pricing_model` 引用的价目
  条目每次同步用网络最新价覆盖四档单价（关联价保持新鲜），其余已存在条目不覆盖
  （人工维护优先），缺失条目补插；同步结果展示新增/关联刷新/未覆盖计数。
