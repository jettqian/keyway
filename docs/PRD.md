...
> 变更记录：
> - v1.5.45：新增**渠道×模型熔断器**——旧失败切换是按请求重放的：第一个渠道挂掉后，
>   每个新请求都会先在它身上耗尽整组尝试再切到备用渠道，既浪费上游请求，也令流量
>   在渠道间反复振荡、拆散上游侧 prompt 缓存（命中率下降）。现按「渠道 × 模型」
>   维度熔断（FR-B1~B6）：连续 3 个请求（`KEYWAY_BREAKER_FAIL_THRESHOLD`）把该渠道
>   该模型的组合全部耗尽（网络错误/HTML 回退/5xx/401/403/429，含 404 model_not_found
>   类模型维度故障）→ 熔断，路由**整渠道跳过**该模型、流量**长期稳定**走备用渠道
>   （不做流量试探——试探弹跳恰好破坏粘性）；切回只经两条路径：后台探测成功
>   自动关闭——矩阵覆盖渠道模型列表前 3 个（「测试渠道」按钮同源、点击即联动
>   恢复），熔断中的模型另做**定向补测**（仅当矩阵存在健康组合时，每个熔断模型
>   在首个健康组合上补发一次最小探测，每轮每渠道上限 10 个、最久未测优先轮转），
>   渠道恢复后最长一个探测周期内全部熔断自动恢复——或前端手动恢复；该模型全部
>   候选渠道都熔断时旁路熔断照常尝试（可用性优先，行为与无熔断一致，旁路期间
>   真实流量成功同样关闭熔断）。渠道列表新增「熔断」列（红标 + 明细弹层：
>   模型/连续失败/熔断时间/最近错误），支持**单个模型或整渠道手动恢复**
>   （`GET /api/breakers`、`POST /api/breakers/reset`，仅渠道所有者可见可操作）；
>   新增 `breaker_states` 表。**探测默认频率 10 → 30 分钟**（探测承担熔断自动
>   恢复职责后以 30 分钟为节拍，环境变量可覆盖）
...
- FR-B1 **熔断**：同一（渠道, 模型）连续 N 个请求（默认 3，`KEYWAY_BREAKER_FAIL_THRESHOLD`）
  ...
  后仍耗尽）/上游 404（模型或端点不存在）——即熔断；路由**整渠道跳过**该模型，不再浪费
  尝试、流量**长期稳定**停留在备用渠道（保持上游缓存命中率）；其余 4xx（400/413 等
  客户端侧问题）不计失败
- FR-B2 **恢复路径（不做流量试探）**：切回不经过真实流量——熔断期间流量始终停留在
  备用渠道（试探弹跳会重新拆散两侧缓存，违背熔断初衷）；关闭熔断只有两条路径：
  ① 后台探测成功（「测试渠道」按钮走同一探测矩阵，点击即联动恢复）——矩阵
  覆盖渠道模型列表前 3 个；对熔断中的模型做**定向补测**：仅当矩阵存在健康组合时
  （渠道整体不可用时补测必然失败，不发无谓请求），每个熔断模型在首个健康组合上
  补发一次最小流式探测，成功即关闭（每轮每渠道上限 10 个，按最久未测优先轮转，
  长期死模型不阻塞其他模型）——渠道恢复后最长一个探测周期内全部熔断自动恢复；
  ② 前端手动恢复（FR-B6）
- FR-B3 **全候选熔断旁路**：该模型的全部候选渠道都处于熔断时，忽略熔断照常尝试——
  可用性优先（此时无处可切，行为与无熔断一致），旁路尝试成功即自动关闭熔断
- FR-B4 **恢复信号**：探测成功（`probe.Result.Model` 记录判定健康的模型）关闭对应
  熔断；全候选旁路期间的真实流量成功（2xx/3xx）同样关闭
- FR-B5 **可见性**：渠道列表新增「熔断」列（红色标签显示熔断模型数），点击弹出明细：
  模型、连续失败次数、熔断时间、最近错误摘要
...
**需求**：
- FR-S1 后台探测：对每个启用渠道的全部"线路 × 路径"组合周期性探测（**默认 30 分钟**，
  可配；v1.5.45 起周期由 10 分钟放宽——探测承担熔断自动恢复职责（FR-B4）后以
  30 分钟为节拍，平衡恢复速度与探测开销），记录延迟与通/断；
  探测矩阵上限 = 5 线路 × 4 路径，超出部分截断（直连与个人代理
...
| 探测开销 | 后台探测产生的上游请求 ≤ 每渠道每小时 120 次（矩阵上限 + 熔断定向补测 × 频率约束）；v1.5.45 默认 30 分钟周期 ≈ 矩阵 40 + 补测 ≤20 次/小时 |
...
| A21 | 渠道×模型熔断 | 渠道 1 对某模型连续 3 个请求失败后，后续请求不再尝试渠道 1（上游命中数不增）、由渠道 2 无感承接；渠道列表可见熔断标签与明细，手动恢复或探测成功后下一个请求切回渠道 1；同渠道其他模型不受影响；唯一渠道熔断时仍照常尝试（旁路） |
