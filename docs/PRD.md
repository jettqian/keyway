...
- FR-L3 用户页：请求数、token 数（汇总条紧凑单位 K/M/B，3 位有效数字，悬停显示
  ...
  **按推理强度分组**（v1.5.53）：按 low/medium/high/`thinking:N` 等强度维度聚合
  tokens 与费用（未开启/历史数据归 `-` 行），日志页明细表新增「推理强度」列；
...
- FR-M3 **用量/花费统计（重点）**：
  - 维度：按 用户 / 模型 / 渠道 / 天 聚合；时间范围可选（今天/7天/30天/自定义）；
    **按用户聚合含零用量用户补齐**（从 users 表回填，未产生过请求的成员以 0 值行可见，
    v1.5.31 实现；管理页按用户/按渠道/按模型/按推理强度四表 + CSV 已实现，按天聚合
    与时间序列图尚未实现，v1.5.52 补齐管理页按渠道表、v1.5.53 增加按推理强度）
