...
| A25 | 最近生效流量 | 统计页显示最近生效流量：同渠道同模型只占一行（取该组合最新一条成功请求），最多 5 个组合；每行按 时间 → 线路 → 渠道 → 模型 → 状态 展示，其中线路为**实际路由线路（域名 + 路径分层短显，悬停完整地址，历史日志无线路数据时显示 —）**；发送新请求后刷新页面即更新（新组合置顶，已有组合更新时间与线路） |
