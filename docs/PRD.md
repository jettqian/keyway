...
> 变更记录：
> - v1.5.14：价目表与模型目录体验增强——① 模型目录（用户页与管理员页）单价列直接显示
>   **完整四档价**：主行输入/输出，副行缓存读/写（未配置标注"同输入"，价目表无同名
>   条目显示"未定价"）；② 管理页价目表：**模型目录中的条目置前显示**（带"目录"标签），
>   新增**按模型名搜索筛选**（含计数）；③ 价目表展示**远程同步元信息**：最近一次官方
>   价目同步时间（`settings.pricing_synced_at`，至少单源成功才记录）与同步源链接
...
- FR-MC1 目录管理（管理员）：模型目录 CRUD（名称、备注、启用）；支持**从价目表一键导入**
  缺失的模型名（幂等，已存在的跳过）；停用的目录项不再出现在用户点选项中。价目表内容
  由管理员/运维按官方价格维护（无自动同步）
- FR-MC2 目录可见性（用户）：启用中的目录模型对所有登录用户只读可见（名称、备注、
  是否已在自己渠道中使用、**关联单价**——按模型名精确匹配 model_pricing，与费用计算
  同口径；直接显示完整四档价（输入/输出/缓存读/写，缓存档未配置标注回退输入价），
  无条目显示"未定价"）
...
- FR-T2 令牌可选：**限定渠道集合（多选 ≤20，配合渠道启用开关按需切换路由）**、
  限定模型（前缀通配）、过期时间
- FR-T2.1 令牌列表的"限定渠道"列**就地弹出渠道配置面板**（Popover，点击摘要打开）：
  面板内以**"限定渠道范围"主开关**控制——关闭 = 不限（路由到所有启用渠道，按渠道优先级）；
  打开 = 默认全选（按渠道优先级排序）后由用户逐渠道关闭/移出。限定状态下已选渠道可
  **拖动手柄**调整顺序（绑定顺序即该令牌的路由优先级，自上而下依次尝试，带落点指示），
  与全局渠道优先级独立、互不影响其他令牌；全部移出后自动回到不限状态。列摘要显示
