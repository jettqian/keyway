...
- FR-L1 每请求记录：时间、用户、令牌、命中渠道（若复制自模板则记录来源模板 id）、
  **命中线路与路径（直连/哪个代理）**、**命中密钥**、协议、模型、上游状态码、耗时
  ...
  错误摘要（截断 512B）。**逐次上游尝试各记一条**（v1.5.42）：失败切换过程中每个
  失败组合（渠道×线路×密钥）各落一条失败日志（错误摘要含上游响应体 message，
  格式与探测摘要一致），最终成功的那次尝试另行一条；未发出上游请求的错误
  （请求体非法、无候选渠道）不落日志
...
- FR-M3 **用量/花费统计（重点）**：
  - 维度：按 用户 / 模型 / 渠道 / 天 聚合；时间范围可选（今天/7天/30天/自定义）；
    **按用户聚合含零用量用户补齐**（从 users 表回填，未产生过请求的成员以 0 值行可见，
    v1.5.31 实现；管理页按用户/按渠道/按模型三表 + CSV 已实现，按天聚合与时间序列图
    尚未实现，v1.5.52 补齐管理页按渠道表）
