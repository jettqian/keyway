...
- FR-T2.1 令牌列表的"限定渠道"列以**胶囊按钮**呈现摘要（边框 + 展开箭头 + hover 反馈），
  ...
  关闭 = 不限（路由到所有启用渠道，按渠道优先级）；打开 = 恢复上次使用的集合与顺序
  （会话级记忆，仅存内存；从未配置过才默认全选，按渠道优先级排序）。限定状态下已选
  渠道**整行可拖拽**（按指针在行内上/下半区计算插入位置，末行下半区即落末尾，带落点
  指示）或用 **↑↓ 按钮**逐位调整顺序（绑定顺序即该令牌的路由优先级，自上而下依次
  尝试），与全局渠道优先级独立、互不影响其他令牌；**移出渠道后重新加入时按记忆位置
  原位恢复**（开关不改变优先级）；全部移出后自动回到不限状态。列摘要显示前两个渠道名
  + 总数；已吊销令牌仅显示摘要不可配置
...
| A24 | 令牌渠道控制 | 令牌渠道面板关闭某渠道开关后该令牌不再路由到它；拖拽整行或 ↑↓ 改变顺序后，下一个请求按新顺序选择候选渠道，其他令牌不受影响；移出后重新加入的渠道回到原优先级位置，主开关关-开后恢复上次集合与顺序 |
