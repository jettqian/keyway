...
> 变更记录：
> - v1.1：① 新增用户级统一模型目录与模型-渠道绑定，可批量维护模型名并按绑定优先级路由；②
...
2. 转发：把请求按模型名路由到该用户自己的某个渠道，含双向协议转换
...
- G9 预制渠道：管理员模板共享**配置**不共享**密钥**；用户一键复制、贴 key、可改任何
  字段；复制后完全独立（见 5.4）
- G10 模型集中管理：模型名只在用户模型目录维护一次，在统一页面批量绑定多个渠道，支持
  每个绑定独立设置上游模型名、启用状态和优先级
...
US13 统一管理模型（P7）
> 我在“模型”页面新增一次 `claude-sonnet-4`，勾选渠道 A、B、C 即可完成绑定；如果三个渠道的
> 上游名称不同，我分别填写映射名。停用某个渠道后它立即不参与该模型路由，恢复后按原优先级
> 自动生效。
...
|---|---|
| name | 展示名 |
| type | `openai`（OpenAI 兼容，覆盖 DeepSeek/GLM/Kimi/Groq/OpenRouter/中转站）或 `anthropic` |
...
- FR-C4 渠道独立控制：渠道详情和模型总览均可单独切换 `enabled`；停用渠道不参与任何模型
  路由、`/v1/models` 聚合和失败切换，已有配置保留，重新启用后恢复。渠道 `priority` 可在
  渠道页或模型页调整，保存后下一个请求生效。

### 5.3.1 统一模型目录

模型目录是用户管理的**逻辑模型名**，与渠道的出站协议和上游模型名解耦。用户不需要在每个
渠道重复创建模型；在模型页创建或导入一次，再批量绑定渠道。

- FR-MD1 每用户可维护 ≤200 个模型：逻辑名（唯一）、展示名、备注、启用开关；逻辑名用于
  Agent 请求和 `/v1/models`，支持从已配置渠道或上游 `models` 接口批量导入并去重。
- FR-MD2 模型详情展示所有渠道绑定；支持批量添加/移除渠道。每条绑定可单独设置上游模型名
  （默认等于逻辑名）、启用开关和路由优先级；同一模型可绑定多个渠道。
- FR-MD3 模型绑定的启用状态与渠道启用状态同时生效；任一关闭即不参与该模型路由，但不删除
  配置。模型总览显示可用渠道数、当前首选渠道和冲突提示。
- FR-MD4 旧版 `channels.models` / `model_mapping` 字段继续读取；保存渠道或编辑模型时迁移为
  模型目录绑定。迁移完成后以模型目录为唯一写入来源，避免两套配置产生分歧。
- FR-MD5 模型目录不承载协议类型。协议由所选渠道决定；用户可在绑定列表看到渠道协议和
  上游模型名，若跨协议使用由网关转换层处理（见 FR-R3）。
...
- FR-X1 模板管理（管理员）：CRUD；字段与渠道一致（type、base_urls、line_strategy、
...
- FR-T4 令牌吊销立即生效
- FR-T5 创建令牌时在成功响应中展示完整密钥；之后用户可在令牌详情点击“查看密钥”，通过当前
  会话和二次确认后再次回看完整值并复制。仅令牌所有者可执行，管理员和其他用户永远
  只能看到前缀；回看不改变密钥，吊销后不可用于请求。

### 5.6 路由与转发

- FR-R1 路由规则：取请求体 `model` → 在该用户启用模型及其启用绑定中筛选启用渠道 → 按
  绑定优先级（未设置时继承渠道 priority）降序取候选；命中多个同优先级时按稳定顺序轮询。
  每个候选绑定携带独立的上游模型名，转发时使用该名称。令牌的渠道限制再与候选集合取交集。
- FR-R2 失败切换（渠道维度）：同模型次优先级渠道，仅当**尚未向客户端写出首字节**时
  允许重试；流式已开始则原样透传错误。线路维度见 FR-S3，密钥维度见 FR-K4/FR-K5
- FR-R3 协议转换（v1 的主要工作量）：
  ...
  - 入站 Anthropic → 出站 openai 型：完整转换（Claude Code 可直连 GLM/DeepSeek 等；
    含 tool use 双向往返、流式增量、thinking/reasoning 映射）
...
- FR-R9 `/v1/models` 返回逻辑模型目录中至少有一个可用绑定的模型；同一逻辑模型只返回一次，
  不暴露渠道密钥或内部优先级。模型请求可以命中多个渠道，失败时按 FR-R2 依次切换。

### 5.6.1 为什么网关仍需要协议类型

Keyway 对 Agent 暴露统一入口，但上游并非只有一种线协议。`openai` / `anthropic` 是渠道的
**出站适配器**：它决定 URL 路径、鉴权头、请求/响应 JSON 结构、SSE 事件和工具调用字段。
同协议请求只需透传和改写鉴权；跨协议时必须按转换表重建请求和流式响应。逻辑模型目录不要求
用户为模型填写协议，用户只需在渠道或预制模板中选择一次；因此“纯转发”描述的是产品职责，
不意味着上游协议可以被忽略。
...
model_channel_bindings(id, model_id, channel_id, upstream_model, priority,
...
| A19 | 统一模型管理 | 用户新增一个逻辑模型后，可在单页批量绑定至少 3 个渠道并分别设置上游模型名；渠道或绑定关闭后不再路由，开启后按优先级恢复；`/v1/models` 只返回一次逻辑模型 |
...
| A22 | 协议适配 | 同协议请求透传；跨 OpenAI/Anthropic 渠道时请求、响应、流式事件和工具调用按转换表工作，模型目录无需重复填写协议 |
