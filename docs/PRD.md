...
- FR-B7 **链路状态**（v1.5.47）：统计页新增「链路状态（渠道 × 模型）」区块——聚合
  时间窗内该组合的全部**上游尝试**（口径与统计一致：失败切换的中间尝试计入），
  每行展示渠道/模型/请求数/错误率/平均延迟/最近请求/当前熔断状态（熔断标签悬停
  显示失败次数、下次试探时间与最近错误）；熔断中的组合即使窗口内零尝试也以零
  尝试行展示（状态可见性不依赖流量）；渠道删除后其链路行不再出现；随统计页
  时间窗联动。管理端统计页提供**全局视角**：聚合**所有用户**的请求，行含渠道
  所有者（多租户 BYOK 下渠道是租户私有资产，全量明细仅管理员可见，普通用户
...
| A28 | 链路状态 | 用户 A 统计页的链路状态只含自己渠道×模型组合（看不到用户 B 的渠道）；管理端统计页为全量视角且每行标注渠道所有者；熔断中的组合在窗口内无流量时仍以零尝试行展示（熔断标签可见）；时间窗切换后请求数/错误率/平均延迟随之变化；删除渠道后其链路行消失 |
