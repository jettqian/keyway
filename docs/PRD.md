...
>   名称跨实例重映射；不含密钥明文的渠道以草稿导入；令牌含明文则按明文重建
>   （跨实例迁移后 Agent 零改配；key_hash 全局唯一，重复导入或他人持有同值令牌时
>   跳过），无明文随机签发新值；条目级问题跳过并计入结果 warnings，整体单事务、
...
- FR-BK2 **导入合并语义**（不删除、不覆盖任何现有配置）：
  - 密钥：同名复用现有（不覆盖值）；含明文值且无同名的创建；无值且无同名的悬空
    （纯结构导出场景，相关渠道绑定随之丢弃）
  - 渠道：一律新建，同名自动加序号后缀（c1 → c1(2)）；密钥绑定按名称重映射，
    悬空引用丢弃；全部密钥悬空的渠道以**草稿**（停用）导入，绑定密钥后手动启用；
    字段校验与手工创建同规则（复用 validateChannel），失败跳过并计入 warnings
  - 令牌：一律新建；含明文的按明文重建（迁移后 Agent 零改配），`key_hash` 全局
    唯一——重复导入或他人持有同值令牌时跳过；无明文的随机签发新值；渠道绑定按
    文件内 id 重映射，引用缺失（渠道被跳过）时丢弃
  - 整个导入**单事务**执行：任一硬错误整体回滚；条目级问题（未命名/校验失败/
    明文格式无效/悬空引用）跳过并计入 warnings；结果以计数（密钥新建/复用、
    渠道新建/草稿、令牌新建）+ 注意事项清单呈现，并提供「刷新页面查看」
...
| A29 | 配置导出/导入 | 完整导出文件包含密钥值/令牌明文/代理地址且弹窗有妥善保管警示；纯结构导出不含任何明文；完整备份导入新实例后原网关令牌直接可用（Agent 零改配）、渠道密钥按名称重映射且请求正常转发；同实例重复导入：同名密钥复用、渠道名加序号后缀、令牌明文已存在跳过且结果有提示；纯结构导入：无明文密钥不创建、渠道以无密钥草稿导入、令牌随机签发新值；非 Keyway 格式或非法 JSON 文件返回 400 |
