...
- FR-R1 路由规则：取请求体 `model` → 在该用户启用渠道中找 `models` 精确命中 → 取
  priority 最高者；命中多个同优先级时轮询（v1 简化为固定顺序）
...
- Q4 协议转换边界清单（Claude 的 cache_control、thinking 块、多模态图片字段在
  openai 侧的取舍）需要在技术方案阶段逐项定义
