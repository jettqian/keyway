...
>   （探测覆盖渠道模型列表前 3 个，「测试渠道」按钮同源、点击即联动恢复）自动
>   关闭，或前端手动恢复；该模型全部候选渠道都熔断时旁路熔断照常尝试（可用性
>   优先，行为与无熔断一致，旁路期间真实流量成功同样关闭熔断）。渠道列表新增
>   「熔断」列（红标 + 明细弹层：模型/连续失败/熔断时间/最近错误），支持**单个
>   模型或整渠道手动恢复**（`GET /api/breakers`、`POST /api/breakers/reset`，
>   仅渠道所有者可见可操作）；新增 `breaker_states` 表。**探测默认频率 10 → 30
>   分钟**（探测承担熔断自动恢复职责后以 30 分钟为节拍，环境变量可覆盖）
...
- FR-B2 **恢复路径（不做流量试探）**：切回不经过真实流量——熔断期间流量始终停留在
  ...
  ① 后台探测成功（探测覆盖渠道模型列表前 3 个；「测试渠道」按钮走同一探测矩阵，
  点击即联动恢复），② 前端手动恢复（FR-B6）。探测覆盖外的模型（第 4 个起）由
  手动恢复兜底
