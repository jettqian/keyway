...
手动收录常用模型；价格模型可关联价目表中任意名称，留空时按目录名称匹配；**关联价目
参与费用计算**（v1.5.63：价目表无目录名条目时按关联名价计费，见 FR-L5）。目录是点选数据源，
...
- FR-L5 费用口径：按**上游实际模型名**（经 model_mapping 映射后）查价目表计算，未命中时
  回退**目录关联价目**（v1.5.63：模型目录中该模型关联的价目条目参与计价，裸名人工条目
  优先），写入日志时固化快照，避免价目调整后历史统计漂移。**缓存计价**（v0.7）：
  ```
