...
- FR-B7 **链路状态**（v1.5.47，v1.5.48 改为同模型对比视图）：统计页新增
  「链路状态（渠道 × 模型）」区块——聚合时间窗内该组合的全部**上游尝试**（口径与
  统计一致：失败切换的中间尝试计入），**按模型分组做同模型横向对比**：组内渠道按
  质量排序（未熔断在前 → 错误率升序 → 延迟升序，熔断沉底），模型按总尝试数降序；
  每行展示渠道/质量分级/请求数/错误率/平均延迟（含相对模型内最优的倍数与着色）/
  最近请求；**质量分级**：优 = 错误率 <5% 且延迟 ≤1.5× 模型内最优、良 = 错误率
  <20% 且 ≤3×、差 = 其余、熔断（悬停显示失败次数/下次试探/最近错误）、样本
  <5 次不评级；样本充足且组内第一的组合标注「最优」。熔断中的组合即使窗口内
  零尝试也以零尝试行展示（状态可见性不依赖流量）；渠道删除后其链路行不再出现；
  随统计页时间窗联动。管理端统计页提供**全局视角**：聚合**所有用户**的请求，行含
  渠道所有者（多租户 BYOK 下渠道是租户私有资产，全量明细仅管理员可见，普通用户
  互不可见）；`GET /api/stats/links`（本人渠道）与
  `GET /api/admin/stats/links`（全量 + 所有者，AdminAuth）
...
| A28 | 链路状态 | 用户 A 统计页的链路状态只含自己渠道×模型组合（看不到用户 B 的渠道）；管理端统计页为全量视角且每行标注渠道所有者；同一模型的多个渠道排在一起且按质量排序，质量最优的组合带「最优」标记，质量分级（优/良/差/熔断）颜色可辨；平均延迟列显示相对模型内最优的倍数；熔断中的组合在窗口内无流量时仍以零尝试行展示（熔断标签可见）；时间窗切换后请求数/错误率/平均延迟随之变化；删除渠道后其链路行消失 |
