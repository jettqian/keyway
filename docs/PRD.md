...
>   类模型维度故障）→ 熔断，冷却 300s（`KEYWAY_BREAKER_COOLDOWN_S`，失败指数退避
>   上限 3600s）内整渠道跳过、流量稳定走备用渠道；冷却到期后放行**单个**组合试探
>   （并发请求原子认领，仅一次）：成功立即关闭、流量切回主渠道，失败重新熔断并加倍
>   冷却；该模型全部候选渠道都熔断时旁路熔断照常尝试（可用性优先，行为与无熔断
>   一致）；真实流量成功与探测成功自动关闭熔断。渠道列表新增「熔断」列（红标 +
>   明细弹层：模型/连续失败/下次试探时间/最近错误），支持**单个模型或整渠道手动
>   恢复**（`GET /api/breakers`、`POST /api/breakers/reset`，仅渠道所有者可见可操作）；
>   新增 `breaker_states` 表。**探测默认频率 10 → 30 分钟**（熔断器接管请求路径的
>   健康反馈后，探测退居线路排序与状态展示，下调以省上游额度，环境变量可覆盖）
...
- FR-B1 **熔断**：同一（渠道, 模型）连续 N 个请求（默认 3，`KEYWAY_BREAKER_FAIL_THRESHOLD`）
  ...
  后仍耗尽）/上游 404（模型或端点不存在）——即熔断；冷却期内（默认 300s，
  `KEYWAY_BREAKER_COOLDOWN_S`）路由**整渠道跳过**该模型，不再浪费尝试、流量稳定停留在
  备用渠道（保护上游缓存命中率）；其余 4xx（400/413 等客户端侧问题）不计失败
- FR-B2 **半开试探**：冷却到期后，下一个请求对熔断渠道仅放行**单个**组合（最优线路 ×
  首选密钥；并发请求通过原子认领保证同一时刻只有一个试探在飞）：成功 → 立即关闭熔断、
  流量切回该渠道（优先级恢复原序）；失败 → 重新熔断并按连续失败次数指数退避
  （冷却 × 2^n，上限 `KEYWAY_BREAKER_COOLDOWN_MAX_S` 默认 3600s）
- FR-B3 **全候选熔断旁路**：该模型的全部候选渠道都处于熔断时，忽略熔断照常尝试——
  可用性优先（此时无处可切，行为与无熔断一致），旁路尝试成功即自动关闭熔断
- FR-B4 **恢复信号**：真实流量成功（2xx/3xx）与后台探测成功（探测覆盖的渠道模型列表
  前 3 个）自动关闭对应熔断；"测试渠道"按钮走同一探测矩阵，同样联动恢复
...
- FR-S1 后台探测：对每个启用渠道的全部"线路 × 路径"组合周期性探测（**默认 30 分钟**，
  可配；v1.5.45 起由 10 分钟下调——熔断器已接管请求路径的健康反馈（失败计数/半开
  恢复），探测退居线路排序与状态展示，并联动关闭熔断（FR-B4）），记录延迟与通/断；
...
| 探测开销 | 后台探测产生的上游请求 ≤ 每渠道每小时 120 次（矩阵上限 × 频率约束）；v1.5.45 默认 30 分钟周期 ≈ 40 次/小时 |
...
| A21 | 渠道×模型熔断 | 渠道 1 对某模型连续 3 个请求失败后，第 4 个请求不再尝试渠道 1（上游命中数不增）、由渠道 2 无感承接；渠道列表可见熔断标签与明细，手动恢复后下一个请求重新尝试渠道 1；同渠道其他模型不受影响；唯一渠道熔断时仍照常尝试（旁路）；冷却到期后放行单次试探，成功即切回 |
