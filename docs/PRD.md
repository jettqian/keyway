...
模型管理页是渠道模型列表的统一入口。模型仍然属于渠道配置：一个渠道可以绑定多个模型；
同一个模型也可以被多个渠道绑定，用于按优先级路由和故障切换。用户不需要逐个打开渠道重复
输入模型名，而是在模型页批量选择渠道。模型列表属于渠道的基础配置，不放入高级设置。

- FR-MD1 模型页展示用户所有渠道的模型并集；支持新增、重命名、删除和批量导入模型名。
- FR-MD2 从模型页选择一个模型后，可批量加入或移出多个渠道；渠道详情页仍可直接编辑该
  渠道的完整模型列表。
- FR-MD3 每个渠道模型绑定可设置上游模型名映射（默认等于请求模型名）；渠道 `enabled`
  和模型绑定状态同时生效，任一关闭即不参与路由，但不删除配置。
- FR-MD4 同一模型命中多个渠道时按渠道 `priority` 降序选择；优先级和渠道开关可在渠道页
  或模型页修改，保存后下一个请求生效。
...
响应、SSE 事件和工具调用。协议类型是转换器的内部输入，不是模型管理页的配置项；同协议渠道
不需要启用转换。
...
| A19 | 统一模型管理 | 用户在单页将一个模型批量加入至少 3 个渠道；每个渠道仍可绑定多个模型，渠道或模型绑定关闭后不再路由，开启后按渠道优先级恢复 |
