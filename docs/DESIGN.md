...
- 版本：v1.45（与 PRD v1.5.48 对应；链路状态改为**同模型对比视图**——原平铺表
...
- 渠道×模型链路状态（`stats.links`，v1.44 / FR-B7）：按（channel_id, model）
  ...
  days）同参联动。**展示为同模型对比视图（v1.45）**：`LinksTable` 按模型分组
  （模型列 rowSpan 合并、组内渠道按质量排序、熔断沉底），质量分级 + 模型内相对
  延迟倍数 + 「最优」标记（规则见版本头，纯前端计算，API 与数据口径不变）
