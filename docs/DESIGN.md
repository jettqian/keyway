...
- 版本：v1.21（与 PRD v1.5.23 对应；探测模型回退（前 3 个模型依序尝试）+
  失败信息携带上游 error.message 摘要 + 组合级总超时（见 §6）；
  前版 v1.20：探测修正——协议判定/模型映射与真实转发对齐（passthrough 按模型名族
  推断 + 失败回退另一协议）、探测矩阵并发化、测试按钮前端即时反馈（见 §6）；
