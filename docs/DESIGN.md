...
- 版本：v1.69（与 PRD v1.5.72 对应；**价目表被关联条目置前**——恢复目录关联排序：
  ...
  PricingTab 移除 `catalogNames` 状态、目录置前排序与「目录」标签，恢复纯模型名排序，
  搜索 + 来源筛选不变）；
  前版 v1.67：与 PRD v1.5.70 对应；**同步恢复自动新增，全量 upsert**——
  ...
  下一请求放行试探」，恢复按钮保留（手动恢复走完整组合计划，与单组合试探
  语义不同）；
  前版 v1.54：与 PRD v1.5.57 对应；日志页列布局瘦身——路径并入线路列
  （LineUrl 的 via 参数）、协议/推理强度并入模型列（模型名下弱化小标签）、
  首字节/总耗时合并「耗时」列（`首 x · 总 y`）、时间列 170→150，13 列变 9 列，
  固定列总宽 ~1000px 常规屏免横向滚动，极窄屏保留横向滚动兜底）；
  前版 v1.53：与 PRD v1.5.56 对应；日志页三处修正——① 每页条数选择器生效：
...
- 全局模型目录 `catalog_models`（管理员手动收录常用模型，用户只关心自己用到的模型，
  ...
  管理页"新增模型"弹窗的模型名为**纯输入框**（v1.5.71：目录与价目是单一映射关系，
  点选交互只在「关联价目」字段表达——该下拉在价目表（`GET /api/admin/pricing`）
  中本地筛选（包含匹配，antd 虚拟滚动），候选项带来源标签与输入/输出价摘要，
  选中后表单下方预览四档价目关联；目录外名称可自由输入。
- 渠道表单的模型候选 = 目录（启用项）∪ 用户已有模型（各渠道 models_json 并集，去重），
  目录外名称仍可自由输入（自定义中转模型名）。
- 用户模型目录提供“选择渠道添加”操作，复用模型绑定原子更新接口批量补充用户勾选的渠道；
  不自动修改其他用户渠道，避免将不被某个渠道支持的模型纳入路由。
...
- **展示**：`GET /api/admin/pricing` 在价目列表外一并返回 `syncedAt` 与 `sources`，
  管理页价目表头部展示"最近同步时间 + 同步源链接"；按模型名排序 + 按模型名搜索筛选
  （v1.5.71：移除目录置前排序与"目录"标签——目录与价目的单一映射关系在模型目录侧表达）。
